...
class LyeCustomGestureRecognizer: UIGestureRecognizer, UIGestureRecognizerDelegate {
    ...
    override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent) {
        ...
        if origin.x < 0 {
            origin.x = 0
        } else if origin.x > UIScreen.main.bounds.width - frame.width {
            origin.x = UIScreen.main.bounds.width - frame.width
        }
        if origin.y < 0 {
            origin.y = 0
        } else if origin.y > UIScreen.main.bounds.height - frame.height {
            origin.y = UIScreen.main.bounds.height - frame.height
        }
